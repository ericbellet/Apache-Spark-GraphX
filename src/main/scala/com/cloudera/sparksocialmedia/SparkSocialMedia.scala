...
sc.parallelize(lib.ShortestPaths.run(grafo,Array(3)).vertices.collect,1).saveAsTextFile(args(6))

println("----------------------LabelPropagation---------------------- \n")
//Invoking LabelPropagation

sc.parallelize(lib.LabelPropagation.run(grafo,5).vertices.collect. sortWith(_._1<_._1),1).saveAsTextFile(args(7))
...
sc.parallelize(fb.take(15),1).saveAsTextFile(args(8))
